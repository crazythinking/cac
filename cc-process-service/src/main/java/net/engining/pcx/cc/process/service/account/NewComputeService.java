...
public class NewComputeService {
	...
	/**
	 * 计算计息截止日期； 如果按日计息的情况下，子账户是“LBAL” 或
	 * 子账户是“LOAN”且当前业务日期不是最后一期的到期日，计息截止日就是当前业务日期+1； 否则就是当前业务日期；
	 * 
	 * @param cactAccount
	 * @param cactSubAcct
	 * @param paymentPlan
	 ...
	 * @return
	 */
	public LocalDate calcEndDate(CactAccount cactAccount, CactSubAcct cactSubAcct, PaymentPlan paymentPlan, LocalDate endDate,
	...
	public SubAcct retrieveSubAcct(String subAcctParamId, CactAccount cactAccount) {
		SubAcct subAcct = parameterCacheFacility.getParameter(SubAcct.class, subAcctParamId, cactAccount.getSetupDate());
	...
	public InterestTable retrieveInterestTable(String interestCode, CactAccount cactAccount) {
		...
		if (account.intParamBaseType != null && account.intParamBaseType.equals(ParamBaseType.Fixed)) {
			interestTable = parameterCacheFacility.getParameter(InterestTable.class, interestCode, cactAccount.getSetupDate());
		}
		else {
			interestTable = parameterCacheFacility.getParameter(InterestTable.class, interestCode,
					systemStatusFacility.getSystemStatus().businessDate);
		}
	...
	public Date getNextPaymentDay(CactAccount cactAccount) {
		...
		/**
		 * //根据账户属性中的还款日类型，决定哪天作为最后还款日 switch (account.paymentDueDay){
		 * //最后还款日类型为账单日后的固定天数 //最后还款日= 上一账单日 + 固定天数 case D: paymentDay =
		 * DateUtils.addDays(cactAccount.getNextStmtDate(),
		 * account.pmtDueDays);break;
		 * 
		 * //最后还款日类型为账单日后固定日期 case F: paymentDay =
		 * DateUtils.setDays(cactAccount.getNextStmtDate(), account.pmtDueDays);
		 * while (paymentDay.before(cactAccount.getNextStmtDate())){ paymentDay
		 * = DateUtils.addMonths(paymentDay, 1); } break;
		 * 
		 * //找不到值抛异常 default : throw new
		 * IllegalArgumentException("账户属性中还款日类型不正确"); }
		 */
	...
	public Date getNextDDDay(CactAccount cactAccount) {
		...
		Date ddDay = null;
		/*
		 * ddDay = DateUtils.addDays(getNextPaymentDay(cactAccount), -
		 * account.directDbDays);
		 */
		return ddDay;
	}
	...
	public BigDecimal getStmtBal(CactSubAcct cactSubAcct) {
		...
		BigDecimal totalBalance = new JPAQueryFactory(em).select(qCactSubAcct.endDayBal.sum().coalesce(BigDecimal.ZERO)).from(qCactSubAcct)
				.where(qCactSubAcct.acctSeq.eq(cactSubAcct.getAcctSeq()), qCactSubAcct.subAcctType.ne("LOAN"),
	...
	/**
	 * FIXME 逻辑似乎有问题，待检查； 根据规则确定计算罚息的基础金额
	 * 
	 * @param cactSubAcct
	 *            子账户信息
	 * @param cactAccount
	...
	public BigDecimal getComputePnitBal(CactAccount cactAccount, CactSubAcct cactSubAcct, Account account) {
		BigDecimal pnitBal = BigDecimal.ZERO;
		// 账期余额，不包含未到期，表示存在逾期
		BigDecimal subBal = getStmtBal(cactSubAcct); 
	...
	public BigDecimal getTotalCapi(CactSubAcct cactSubAcct) {
		...
		// (qCactSubAcct.subAcctType.eq("LOAN").or(
		...
		// 重构如下，提高性能
		BigDecimal totalBalance = new JPAQueryFactory(em).select(qCactSubAcct.currBal.sum().coalesce(BigDecimal.ZERO)).from(qCactSubAcct)
				.where(qCactSubAcct.acctSeq.eq(cactSubAcct.getAcctSeq()),
	...
	public Date getNextGraceDay(CactAccount cactAccount) {
		...
		/*
		 * switch (account.paymentDueDay){ //最后还款日类型为账单日后的固定天数或者账单日后固定日期
		 * //最后还款日= 还款日 + 宽限期天数 case D: case F: graceDay =
		 * DateUtils.addDays(paymentDay, account.pmtGracePrd); break;
		 * 
		 * //找不到值抛异常 default : throw new
		 * IllegalArgumentException("账户属性中还款日类型不正确"); }
		 */
	...
	public Date getNextInterstDate(CactAccount cactAccount, Date date, Account acctParam, String billingCycle) {
		...
		if (acctParam.intSettleStartMethod.equals(CycleStartDay.P)) {
			switch (acctParam.intUnit) {
				case D:
					interestDate = DateUtils.addDays(date, acctParam.intUnitMult);
					break;
				case W:
					interestDate = DateUtils.addDays(date, acctParam.intUnitMult * 7);
					break;
				case M:
						...
						if (beginDay != null) {
							...
							if (period == 0) {// 放款的时候,下次计息日的判断
								if (calendar.get(Calendar.DAY_OF_MONTH) <= day || beginDay.equals(cactAccount.getSetupDate())) { // 当日的多次放款
									...
									interestDate = DateUtils.addMonths(calendar.getTime(),
											acctParam.intUnitMult * (cactAccount.getCurrentLoanPeriod() + 1));
								}
								else {
									calendar.set(calendar.get(Calendar.YEAR), calendar.get(Calendar.MONTH), day);
									interestDate = DateUtils.addMonths(calendar.getTime(),
											acctParam.intUnitMult * (cactAccount.getCurrentLoanPeriod() + 2));
								}
							}
	...
	private Date interestDate(Date processDate, Account acctParam, String billingCycle) {
		...
		while (nowDate.compareTo(natDate) >= 0) {
			...
			if (systemStatusFacility.getSystemStatus().systemStatus == SystemStatusType.N && nowDate.compareTo(natDate) == 0) {
				return natDate;
			}
	...
	/**
	 * 第一笔贷款开始日期,根据AcctNo TODO 方法名不合理，待重构
	 * 
	 * @param acctNo
	 * @return
	 */
	public Date getBeginDay(Integer acctNo) {
		...
		List<Date> setupDates = new JPAQueryFactory(em).select(qCactAccount.setupDate).from(qCactAccount)
				.where(qCactAccount.acctNo.eq(acctNo), qCactAccount.businessType.eq(BusinessType.BL)).orderBy(qCactAccount.acctSeq.asc()).fetch();
	...
	/**
	 * 第一笔贷款开始日期,根据AcctSeq TODO acctSeq是主键，查询逻辑不合理，待重构
	 * 
	 * @param acctSeq
	 * @return
	 */
	public Date getBeginDayByAcctSeq(Integer acctSeq) {
		...
		// day=
		// calendar.get(Calendar.DAY_OF_MONTH)>=28?28:calendar.get(Calendar.DAY_OF_MONTH);
		// }
	...
	public BigDecimal calcPrincipalAmt(BigDecimal bal, Integer acctSeq) {
		...
		BigDecimal rate = (dailyRates.get(0).rate).multiply(
				BigDecimal.valueOf(Days.daysBetween(startDate, endDate).getDays() <= 0 ? 0 : Days.daysBetween(startDate, endDate).getDays()));

		BigDecimal principalAmt = bal.divide((new BigDecimal(1).add(rate)), 2, RoundingMode.HALF_UP);
		return principalAmt;
	}
		...
		switch (inttable.rateBaseInterval) {
			...
			case W:
				...
				rate = inttable.chargeRates.get(0).rate
						.multiply(BigDecimal.valueOf(org.annualInterestRateBenchmarkDays).divide(BigDecimal.valueOf(7)));
				break;
			default:
				rate = inttable.chargeRates.get(0).rate;
		}
