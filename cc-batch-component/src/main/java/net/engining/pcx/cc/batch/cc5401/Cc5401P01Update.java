 ...
 * 
 */
public class Cc5401P01Update implements ItemProcessor<ApSubjectSummary, ApSubjectSummary> {
	...
	public ApSubjectSummary process(ApSubjectSummary item) throws Exception {
		...
		ApGlBal agb = new JPAQueryFactory(em)
				.select(qApGlBal)
				.from(qApGlBal)
				.where(qApGlBal.branchNo.eq(item.getOwingBranch()), qApGlBal.subjectCd.eq(item.getSubjectCd()))
				.fetchOne();

		if (agb == null)
			throw new RuntimeException(
					"科目初始化错误, " + "branchNo:" + item.getOwingBranch() + ", titleCd:" + item.getSubjectCd());
